...
//           console.log("hiii jeee",userArray[0][1])
//           const userData = {
      ...
      if (jsonResponse.rStatus === 0 && jsonResponse.rData && jsonResponse.rData.lessons) {
        ...
        if (userArray[0] && userArray.length > 0) {
          ...
          const userData = {
            ...
            religious_beliefs: userArray[0][22] || 'Not specified',
            interests: interestsArray.join(', ') || 'N/A',
            profile_picture: userArray[0][0] || '', // Assuming profile picture is at index 0
          };
    ...
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.profilePictureContainer}>
        {user?.profile_picture ? (
          ...
          <ProfilePicture uri="" /> // Fallback if no profile picture
        )}
      </View>
      <Text style={styles.name}>{user?.full_name}</Text>
      <Text style={styles.bio}>{user?.bio}</Text>
